      ...
      <div className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-12 px-4 sm:px-6 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        >
          ...
          <div className="relative space-y-5 p-5">
            ...
            <div className="rounded-xl border border-base-500/70 bg-base-600/40 p-4 font-mono text-xs">
              <div className="text-ink-400">-- typical workflow</div>
              <div className="mt-3 text-teal">01</div><div className="text-ink-200">clean → validate → explore</div>
              <div className="mt-2 text-teal">02</div><div className="text-ink-200">query → compare → explain</div>
              <div className="mt-2 text-teal">03</div><div className="text-ink-200">visualize → communicate</div>
            </div>
            <div className="rounded-xl border border-base-500/70 bg-base-600/40 p-4">
              <div className="mono-tag mb-3">analysis trend</div>
              <div className="flex h-28 items-end gap-1.5">
                {bars.map((h, i) => <motion.div key={i} initial={{ height: 0 }} animate={{ height: `${h}%` }} transition={{ duration: 0.8, delay: 0.7 + i * 0.05, ease: "easeOut" }} className="flex-1 rounded-t-sm bg-gradient-to-t from-teal/40 to-gold/70" />)}
              </div>
            </div>
            <div className="flex items-center justify-between rounded-xl border border-base-500/70 bg-base-600/40 px-4 py-3 font-mono text-xs text-ink-400">
              <span>SELECT insights FROM analysis</span><span className="text-teal">READY</span>
            </div>
          </div>
        </motion.div>
      </div>
