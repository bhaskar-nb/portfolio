    ...
    <section id="experience" className="relative overflow-hidden py-28">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
          ...
          description="An internship covering data preparation, exploratory analysis, predictive workflows, and interactive application delivery."
        />
            ...
            <div className="relative overflow-hidden border-b border-base-500/70 p-7 sm:p-9 lg:border-b-0 lg:border-r lg:p-10">
              ...
              <div className="relative flex h-full flex-col">
                ...
                <div className="mt-7 flex flex-wrap gap-2">
                  {["Python", "Pandas", "Scikit-learn", "Streamlit"].map((tech) => (
                ...
                <div className="mt-auto flex items-center justify-between gap-4 pt-10 text-xs text-ink-600">
                  <span className="flex items-center gap-2"><Layers3 size={14} className="text-gold" aria-hidden="true" /> Data → analysis → application</span>
            ...
            <div className="p-7 sm:p-9 lg:p-10">
              ...
              <div className="mt-7 space-y-3">
                {experience.bullets.map((item, i) => (
                  <motion.div key={item} initial={{ opacity: 0, x: 14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: i * 0.1 }} className="group flex gap-4 rounded-2xl border border-base-500/70 bg-base-700/30 p-5 transition-all duration-300 hover:border-gold/35 hover:bg-base-700/60">
                    <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-gold/20 bg-gold/5 text-gold"><Check size={14} aria-hidden="true" /></span>
                    <div><span className="font-mono text-[9px] uppercase tracking-[0.14em] text-ink-600">0{i + 1}</span><p className="mt-1.5 text-sm leading-7 text-ink-300">{item}</p></div>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="mt-8 border-t border-base-500/70 pt-7">
                <div className="flex items-end justify-between gap-4">
                  <div><span className="font-mono text-[9px] uppercase tracking-[0.2em] text-gold">workflow</span><p className="mt-2 text-sm text-ink-400">The working pattern behind the internship</p></div>
                  <ArrowUpRight size={16} className="text-ink-600" aria-hidden="true" />
                </div>

                <div className="mt-5 grid gap-3 sm:grid-cols-3">
                  {workflow.map(({ icon: Icon, step, label, detail, signal }) => (
                    <motion.div key={label} whileHover={{ y: -3 }} transition={{ duration: 0.2 }} className="group rounded-2xl border border-base-500/70 bg-base-700/20 p-4 transition-colors duration-300 hover:border-gold/30 hover:bg-base-700/50">
                      <div className="flex items-center justify-between"><Icon size={16} className="text-gold" aria-hidden="true" /><span className="font-mono text-[9px] text-ink-600">{step}</span></div>
                      <p className="mt-4 text-sm font-medium text-ink-200">{label}</p><p className="mt-1 text-[11px] leading-5 text-ink-600">{detail}</p><p className="mt-3 border-t border-base-500/60 pt-3 font-mono text-[8px] uppercase tracking-[0.12em] text-gold/70">{signal}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
