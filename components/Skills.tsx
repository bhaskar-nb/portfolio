...
const sections = [
  ...
  { id: "database", title: "DATABASE", caption: "Structured data", icon: Database, skills: getSkills("database"), span: "lg:col-span-2" },
  ...
  return (
    <section id="skills" className="relative overflow-hidden py-28">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading eyebrow="-- skills" title="A stack built for the full analysis loop" description="The tools I use to move from structured questions and raw tables to analysis, visualization, and communication." />
            ...
            return (
              <motion.div key={section.id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-70px" }} transition={{ duration: 0.5, delay: (index % 3) * 0.07 }} whileHover={{ y: -4 }} className={`group relative overflow-hidden rounded-2xl border border-base-500/80 bg-base-800/60 p-6 transition-all duration-300 hover:border-gold/40 hover:bg-base-700/70 ${section.span}`}>
                ...
                <div className="relative flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl border border-base-500 bg-base-700/60 text-gold"><Icon size={16} /></span><div><span className="block font-mono text-[10px] uppercase tracking-[0.18em] text-ink-400">{section.title}</span><span className="mt-1 block text-[11px] text-ink-600">{section.caption}</span></div></div>
                  <ArrowUpRight size={15} className="text-ink-600 transition-all group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-gold" aria-hidden="true" />
                </div>
                <div className="relative mt-7 flex flex-wrap gap-2.5">
                  {section.skills.map((skill, skillIndex) => <motion.div key={skill} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: skillIndex * 0.035 }} className="inline-flex items-center gap-2 rounded-xl border border-base-500/70 bg-base-700/40 px-3.5 py-2.5 text-sm text-ink-200 transition-all duration-300 hover:-translate-y-0.5 hover:border-gold/50 hover:bg-base-600/50 hover:text-ink-100"><BrandIcon skill={skill} /><span>{skill}</span></motion.div>)}
                </div>
              </motion.div>
            );
          })}
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-5 overflow-hidden rounded-2xl border border-gold/20 bg-gold/[0.03] p-5 sm:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div><span className="font-mono text-[9px] uppercase tracking-[0.2em] text-gold">analysis workflow</span><p className="mt-2 max-w-md text-sm leading-6 text-ink-400">I treat the stack as a workflow, not a list of disconnected tools.</p></div>
            <div className="flex flex-wrap items-center gap-2 font-mono text-[9px] uppercase tracking-wider text-ink-500">{["question", "raw data", "SQL", "clean", "analyze", "visualize", "insight"].map((step, i) => <span key={step} className="flex items-center gap-2"><span className={i === 0 || i === 6 ? "text-gold" : ""}>{step}</span>{i < 6 && <span className="text-base-400">→</span>}</span>)}</div>
          </div>
        </motion.div>
      </div>
    </section>
