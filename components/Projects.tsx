...
const previewMeta: Record<string, { label: string; metrics: string[]; values: number[]; bars: number[] }> = {
  "sales-performance-dashboard": { label: "sales / performance", metrics: ["Sales YoY", "Profit", "Growth"], values: [82, 91, 76], bars: [42, 58, 48, 68, 61, 78, 70, 88, 79, 96] },
  "global-disaster-analysis": { label: "disasters / global", metrics: ["Events", "Regions", "Trend"], values: [74, 61, 88], bars: [72, 55, 64, 48, 78, 69, 84, 63, 91, 76] },
  "hr-analytics-dashboard": { label: "people / analytics", metrics: ["Headcount", "Attrition", "Satisfaction"], values: [69, 43, 82], bars: [54, 62, 48, 71, 66, 58, 77, 64, 72, 86] },
  "ev-adoption-dashboard": { label: "mobility / adoption", metrics: ["EV Records", "Adoption", "Trend"], values: [90, 72, 84], bars: [38, 51, 59, 66, 63, 74, 79, 83, 88, 94] },
};
    ...
    <section id="projects" className="relative overflow-hidden py-28">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-12 space-y-6">
          {projects.map((p, i) => {
            const flagship = i === 0;
            const preview = previewMeta[p.id] ?? previewMeta["sales-performance-dashboard"];
            return (
              <motion.button key={p.id} type="button" onClick={() => setActive(p)} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6, delay: i * 0.06 }} whileHover={{ y: -4 }} className={`group relative grid min-h-[360px] w-full overflow-hidden rounded-3xl border bg-base-800/70 text-left backdrop-blur-sm transition-all duration-500 md:grid-cols-[1.05fr_0.95fr] ${flagship ? "border-gold/50 shadow-[0_0_60px_rgba(34,197,94,0.08)] hover:border-gold/80" : "border-base-500/80 hover:border-gold/50"}`}>
                ...
                <div className="relative flex min-h-[260px] items-center justify-center overflow-hidden bg-gradient-to-br from-base-700 to-base-900 p-6 sm:p-8">
                  <div className="absolute inset-0 bg-radial-fade opacity-60 transition-opacity duration-500 group-hover:opacity-100" />
                  <div className="relative w-full max-w-sm overflow-hidden rounded-2xl border border-base-400/70 bg-base-800/90 p-4 shadow-2xl transition-transform duration-700 group-hover:scale-[1.03] group-hover:-rotate-1">
                    <div className="flex items-center justify-between border-b border-base-500/70 pb-3"><div><span className="block font-mono text-[9px] uppercase tracking-widest text-ink-500">{preview.label}</span><span className="mt-1 block text-[8px] uppercase tracking-wider text-ink-600">verified project view</span></div><span className="h-1.5 w-1.5 rounded-full bg-gold shadow-[0_0_12px_rgba(34,197,94,0.8)]" aria-hidden="true" /></div>
                    <div className="grid grid-cols-3 gap-2 py-4">{preview.metrics.map((label, index) => <div key={label} className="rounded-lg border border-base-500/70 bg-base-700/60 p-2.5"><span className="block truncate font-mono text-[8px] uppercase text-ink-600">{label}</span><span className="mt-2 block font-display text-sm text-ink-200">{index === 0 && p.id === "sales-performance-dashboard" ? "20.6%" : index === 1 && p.id === "sales-performance-dashboard" ? "43.7%" : `${preview.values[index]}%`}</span><span className="mt-1 block h-1 rounded-full bg-base-500"><span className="block h-1 rounded-full bg-gold/70" style={{ width: `${preview.values[index]}%` }} /></span></div>)}</div>
                    <div className="rounded-lg border border-base-500/60 bg-base-900/50 p-3"><div className="mb-2 flex items-center justify-between"><span className="font-mono text-[8px] uppercase tracking-widest text-ink-600">trend / comparison</span><span className="font-mono text-[8px] text-gold">+ signal</span></div><div className="flex h-24 items-end gap-1.5">{preview.bars.map((height, index) => <motion.span key={index} initial={{ height: 0 }} whileInView={{ height: `${height}%` }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.04 }} className="flex-1 rounded-t-sm bg-gold/60" aria-hidden="true" />)}</div></div>
                    <div className="mt-3 flex items-center justify-between"><span className="font-mono text-[8px] uppercase tracking-[0.16em] text-ink-600">question → evidence → decision</span><ArrowUpRight size={12} className="text-gold" aria-hidden="true" /></div>
                  </div>
              ...
              return (
                <motion.button key={p.id} type="button" onClick={() => setActiveAdditional(p)} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5, delay: i * 0.07 }} whileHover={{ y: -5 }} className="group relative overflow-hidden rounded-2xl border border-base-500/80 bg-base-800/60 p-6 text-left transition-all duration-300 hover:border-gold/50 hover:bg-base-700/70 sm:p-7">
                  ...
                  <div className="relative">
                    <div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-base-500 bg-base-700/70 text-gold"><Icon size={17} aria-hidden="true" /></span><div><span className="font-mono text-[9px] uppercase tracking-[0.16em] text-gold">0{i + 1} / {p.category}</span><span className="mt-1 block font-mono text-[9px] uppercase tracking-wider text-ink-600">{meta?.signal}</span></div></div><ArrowUpRight size={17} className="mt-1 shrink-0 text-ink-600 transition-all group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-gold" aria-hidden="true" /></div>
                    <h3 className="mt-8 max-w-lg font-display text-2xl tracking-tight text-ink-100">{p.title}</h3><p className="mt-3 max-w-xl text-sm leading-6 text-ink-400">{p.summary}</p>
