          ...
          {projects.map((p, i) => {
            ...
            return (
              <motion.button
                ...
                whileHover={{ y: -4 }}
                className={`group relative grid min-h-[360px] w-full overflow-hidden rounded-3xl border bg-base-800/70 text-left backdrop-blur-sm transition-all duration-500 md:grid-cols-[1.05fr_0.95fr] ${flagship ? "border-gold/50 shadow-[0_0_60px_rgba(255,122,24,0.08)] hover:border-gold/80" : "border-base-500/80 hover:border-gold/50"}`}
              >
                <div className="relative flex flex-col justify-between overflow-hidden border-b border-base-500/70 p-7 sm:p-9 md:border-b-0 md:border-r">
                  <div>
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-gold">{flagship ? "01 / Flagship case study" : `0${i + 1} / Featured`}</span>
                      {flagship && <span className="rounded-full border border-gold/30 bg-gold/10 px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.16em] text-gold">Business lens</span>}
                ...
                <div className="relative flex min-h-[260px] items-center justify-center overflow-hidden bg-gradient-to-br from-base-700 to-base-900 p-8">
                  ...
                  <div className="relative w-full max-w-sm rounded-2xl border border-base-400/70 bg-base-800/80 p-4 shadow-2xl transition-transform duration-700 group-hover:scale-[1.03] group-hover:-rotate-1">
                    <div className="flex items-center justify-between border-b border-base-500/70 pb-3"><span className="font-mono text-[9px] uppercase tracking-widest text-ink-500">analytics / dashboard</span><span className="h-1.5 w-1.5 rounded-full bg-gold shadow-[0_0_12px_rgba(255,122,24,0.8)]" aria-hidden="true" /></div>
                    <div className="grid grid-cols-3 gap-2 py-4">
                      {["Revenue", "Profit", "Growth"].map((label, index) => <div key={label} className="rounded-lg border border-base-500/70 bg-base-700/60 p-2.5"><span className="block font-mono text-[8px] uppercase text-ink-600">{label}</span><span className="mt-2 block h-2 rounded-full bg-base-500"><span className="block h-2 rounded-full bg-gold/70" style={{ width: `${55 + index * 15}%` }} /></span></div>)}
                    </div>
                    <div className="flex h-24 items-end gap-2 rounded-lg border border-base-500/60 bg-base-900/50 p-3">{[35, 52, 42, 68, 58, 78, 72, 90, 76, 96].map((height, index) => <motion.span key={index} initial={{ height: 0 }} whileInView={{ height: `${height}%` }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.04 }} className="flex-1 rounded-t-sm bg-gold/60" />)}</div>
                  </div>
      ...
      <AnimatePresence>
        {active && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] flex items-center justify-center bg-base-900/85 p-4 backdrop-blur-md" onClick={() => setActive(null)} role="dialog" aria-modal="true" aria-labelledby="project-case-study-title"><motion.div initial={{ opacity: 0, scale: 0.94, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 12 }} transition={{ duration: 0.28 }} onClick={(e) => e.stopPropagation()} className="max-h-[88vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-base-400/80 bg-base-700 shadow-2xl">
          ...
          <div className="space-y-7 p-6 sm:p-8">
            <div><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-[10px] uppercase tracking-[0.18em] text-gold">{active.id === "sales-performance-dashboard" ? "Flagship case study" : "Selected project"}</span><span className="text-ink-600">/</span><span className="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-500">{active.stack.join(" · ")}</span></div><h3 id="project-case-study-title" className="mt-3 font-display text-3xl text-ink-100 sm:text-4xl">{active.title}</h3><p className="mt-5 text-base leading-7 text-ink-300">{active.summary}</p></div>
            <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-base-500/70 bg-base-800/50 p-4"><div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-ink-200"><Target size={15} className="text-gold" aria-hidden="true" /> Question</div><p className="mt-3 text-sm leading-6 text-ink-400">{active.challenges}</p></div><div className="rounded-2xl border border-base-500/70 bg-base-800/50 p-4"><div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-ink-200"><Workflow size={15} className="text-gold" aria-hidden="true" /> Approach</div><p className="mt-3 text-sm leading-6 text-ink-400">{active.approach}</p></div></div>
