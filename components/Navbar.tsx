      ...
      <div className={cn("mx-auto max-w-7xl px-4 transition-all duration-500 sm:px-6 lg:px-8", scrolled ? "pt-3" : "pt-5")}>
        <div className={cn("pointer-events-auto relative flex items-center justify-between overflow-hidden rounded-full border px-3 py-2", "border-base-500/80 bg-base-800/75 backdrop-blur-xl", "shadow-[0_12px_50px_-20px_rgba(0,0,0,0.7)] transition-all duration-500", scrolled ? "mx-auto max-w-5xl" : "max-w-7xl")}>
          ...
          <nav aria-label="Primary navigation" className="hidden items-center gap-0.5 lg:flex">
            {primaryNav.map((item) => <button key={item.id} onClick={() => scrollTo(item.id)} className={cn("relative min-h-11 rounded-full px-4 py-2 font-mono text-[10px] uppercase tracking-[0.14em] transition-colors", active === item.id ? "text-base-900" : "text-ink-400 hover:text-ink-100")}>
        ...
        <AnimatePresence>{open && <motion.div id="mobile-navigation" initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ duration: 0.2 }} className="pointer-events-auto mx-1 mt-2 overflow-hidden rounded-2xl border border-base-500/80 bg-base-800/95 p-2 shadow-2xl backdrop-blur-xl lg:hidden">
          ...
          <a href={resumeUrl} download onClick={() => setOpen(false)} className="mt-1 flex min-h-11 items-center justify-center gap-2 rounded-xl border border-base-500 px-4 py-3.5 font-mono text-xs uppercase tracking-[0.14em] text-gold"><Download size={14} /> Download Resume</a>
