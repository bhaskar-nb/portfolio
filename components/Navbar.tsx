      ...
      <div className="mx-auto max-w-7xl px-4 pt-4 sm:px-6 sm:pt-5 lg:px-8">
        <div className="pointer-events-auto mx-auto flex min-h-[62px] items-center justify-between rounded-2xl border border-base-500/70 bg-base-900/75 px-3 shadow-[0_18px_50px_rgba(0,0,0,0.28)] backdrop-blur-xl sm:px-4">
          <button type="button" onClick={() => scrollTo("home")} className="group flex min-h-11 items-center gap-3 rounded-xl px-2 transition-colors hover:bg-base-700/50" aria-label="Go to home">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl border border-violet-400/30 bg-violet-500/10 font-display text-sm font-semibold tracking-[-0.06em] text-ink-100 transition-transform duration-300 group-hover:-translate-y-0.5">
              B<span className="text-violet-300">N</span>
            </span>
          ...
          <nav aria-label="Primary navigation" className="hidden items-center lg:flex">
            {primaryNav.map((item) => (
              <button type="button" key={item.id} onClick={() => scrollTo(item.id)} className={cn("group relative flex min-h-11 items-center px-3.5 font-mono text-[9px] uppercase tracking-[0.13em] transition-colors duration-200", active === item.id ? "text-ink-100" : "text-ink-500 hover:text-ink-200")}>
                ...
                <span className={cn("absolute bottom-1.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-violet-500 opacity-0 transition-all duration-200", active === item.id ? "opacity-100" : "group-hover:opacity-50")} />
              </button>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <a href="#contact" className="group hidden min-h-11 items-center gap-1.5 rounded-xl bg-violet-500 px-3.5 font-mono text-[9px] font-medium uppercase tracking-[0.13em] text-base-900 shadow-[0_0_22px_rgba(139,92,246,0.14)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_0_28px_rgba(139,92,246,0.24)] sm:flex">
            ...
            <motion.div id="mobile-navigation" initial={{ opacity: 0, y: -10, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.98 }} transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }} className="pointer-events-auto mt-2 lg:hidden">
              <div className="overflow-hidden rounded-2xl border border-base-500/70 bg-base-900/90 p-2 shadow-[0_18px_50px_rgba(0,0,0,0.35)] backdrop-blur-xl">
                <div className="grid grid-cols-2 gap-1">
                  {primaryNav.map((item) => (
                    <button type="button" key={item.id} onClick={() => scrollTo(item.id)} className={cn("flex min-h-12 items-center justify-between rounded-xl px-4 font-mono text-[10px] uppercase tracking-[0.13em] transition-colors", active === item.id ? "bg-violet-500 text-base-900" : "text-ink-400 hover:bg-base-700/70 hover:text-ink-100")}>
                      {item.label}
                ...
                <div className="mt-2 grid grid-cols-2 gap-1 border-t border-base-500/60 pt-2">
                  ...
                  <a href="#contact" onClick={() => setOpen(false)} className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-violet-500 font-mono text-[10px] font-medium uppercase tracking-[0.13em] text-base-900"><MessageCircle size={13} aria-hidden="true" /> Let&apos;s talk</a>
                </div>
              </div>
            </motion.div>
          )}
