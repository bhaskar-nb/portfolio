    ...
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50">
      <div className={cn("mx-auto max-w-7xl px-4 transition-all duration-500 sm:px-6 lg:px-8", scrolled ? "pt-3" : "pt-5")}>
        <div className={cn("pointer-events-auto relative flex items-center justify-between overflow-hidden rounded-full border px-3 py-2", "border-base-500/80 bg-base-800/80 backdrop-blur-xl", "shadow-[0_12px_50px_-20px_rgba(0,0,0,0.7)] transition-all duration-500", scrolled ? "mx-auto max-w-5xl" : "max-w-7xl")}>
          <button type="button" onClick={() => scrollTo("home")} className="group flex min-h-11 items-center gap-3 rounded-full px-3 py-2" aria-label="Go to home">
          ...
          <nav aria-label="Primary navigation" className="hidden items-center gap-0.5 lg:flex">
            {primaryNav.map((item) => (
              <button type="button" key={item.id} onClick={() => scrollTo(item.id)} className={cn("relative min-h-11 rounded-full px-4 py-2 font-mono text-[10px] uppercase tracking-[0.14em] transition-colors", active === item.id ? "text-base-900" : "text-ink-400 hover:text-ink-100")}>
          ...
          <div className="flex items-center gap-2">
            <a href="#contact" className="group hidden min-h-11 items-center gap-1.5 rounded-full bg-gold px-4 py-2 font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-base-900 transition-transform hover:scale-[1.03] sm:flex"><MessageCircle size={12} /> Let&apos;s talk <ArrowUpRight size={11} className="transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></a>
            <a href={resumeUrl} download className="group hidden min-h-11 items-center gap-1.5 rounded-full border border-base-400 px-4 py-2 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-200 transition-all hover:border-gold/60 hover:text-gold md:flex"><Download size={12} /> Résumé</a>
            <button type="button" className="flex h-11 w-11 items-center justify-center rounded-full border border-base-500 text-ink-200 transition-colors hover:border-gold/60 hover:text-gold lg:hidden" onClick={() => setOpen((value) => !value)} aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open} aria-controls="mobile-navigation">{open ? <X size={16} /> : <Menu size={16} />}</button>
          </div>
        </div>
        ...
        <AnimatePresence>
          {open && <motion.div id="mobile-navigation" initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ duration: 0.2 }} className="pointer-events-auto mx-1 mt-2 overflow-hidden rounded-2xl border border-base-500/80 bg-base-800/95 p-2 shadow-2xl backdrop-blur-xl lg:hidden">
            {primaryNav.map((item) => <button type="button" key={item.id} onClick={() => scrollTo(item.id)} className={cn("flex min-h-11 w-full items-center justify-between rounded-xl px-4 py-3.5 text-left font-mono text-xs uppercase tracking-[0.14em] transition-colors", active === item.id ? "bg-gold/10 text-gold" : "text-ink-400 hover:bg-base-700 hover:text-ink-100")}>{item.label}{active === item.id && <span className="h-1.5 w-1.5 rounded-full bg-gold" />}</button>)}
            <div className="mt-1 grid grid-cols-2 gap-2"><a href="#contact" onClick={() => setOpen(false)} className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-gold px-4 py-3.5 font-mono text-xs font-medium uppercase tracking-[0.14em] text-base-900"><MessageCircle size={14} /> Let&apos;s talk</a><a href={resumeUrl} download onClick={() => setOpen(false)} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-base-500 px-4 py-3.5 font-mono text-xs uppercase tracking-[0.14em] text-gold"><Download size={14} /> Résumé</a></div>
          </motion.div>}
        </AnimatePresence>
      </div>
    </header>
